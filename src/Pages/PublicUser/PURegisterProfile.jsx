...
import { registerProfile, resetRegisterProfile, resetRegisterMessages, publicUserDetails } from "../../Features/Slices/PublicUser/publicUserSlice"
...
export function PURegisterProfile() {
  ...
  const { maritalstatus, qualification, additionalQualification, DOB, POB, birthTime, contactPerson, job, salary,
    fatherOccupation, motherOccupation, religion, foodPreference, motherTongue, caste, subcaste, district, region, state, address1,
  ...
  const { broker, isError, isSuccess, isLoading, message, isStarListSuccess,
    ...
    isDistrictListLoading, isDistrictListSuccess, qualifications,
  ...
  const { isRegisterProfileLoading, isRegisterProfileSuccess, isRegisterProfileError, registerProfilemessage, registerProfileId } =
    useSelector(
      (state) => state.public
    )
  ...
  useEffect(() => {
    ...
    if (isRegisterProfileSuccess) {
      ...
      navigate('/PURegisterImage?id=' + registerProfileId + "&name=PUNewProfile", { replace: true });
    }
  ...
  const onSubmit = (e) => {
    ...
    //   job, contactPerson, religion, foodPreference, motherTongue, caste, POB, star, rasi,
    //   address1, qualification, maritalstatus
    // }
    ...
    if (/[a-zA-Z]/.test(weight)) {
      ...
      const profileData = {
        maritalstatus, qualification, additionalQualification, POB, birthTime, contactPerson, job, salary,
        fatherOccupation, motherOccupation, religion, foodPreference, motherTongue, caste, subcaste, state,
      ...
      //   return toast.error("Boy age must be at least 21 years old.");
      // }
      // if (sex === "Female" && calculateAge(DOBBirth) < 18) {
      //   return toast.error("Girls age must be at least 18 years old.");
      // }
  ...
  const { publicuserRegister, isPublicUserDetailLoading, isPublicUserDetailSuccess, publicuserId } = useSelector((state) => state.public)

  useEffect(() => {
    if (isPublicUserDetailLoading == false && isPublicUserDetailSuccess == false) {
      dispatch(publicUserDetails())
    }
  }, [publicuserRegister, isPublicUserDetailLoading, isPublicUserDetailSuccess], dispatch)
  ...
  const { Name, PhoneNumber, District, Sex, DOB1 } = formDatas

  useEffect(() => {
    if (isPublicUserDetailSuccess == true && publicuserRegister) {
      setFormDatas({
        Name: publicuserRegister.name || '',
        PhoneNumber: publicuserRegister.phoneNumber || '',
        District: publicuserRegister.district || '',
        Sex: publicuserRegister.sex || '',
        DOB1: publicuserRegister.DOB || '',
      })
    }
  }, [publicuserRegister, isPublicUserDetailLoading, isPublicUserDetailSuccess], dispatch)
    ...
    <>
      <div>
        ...
        <div className="dropdown-item d-flex align-items-center">
          <svg onClick={goBackClick} style={{ cursor: 'pointer' }} xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="#1aa179" className="bi bi-arrow-left-circle" viewBox="0 0 16 16">
            <path fill-rule="evenodd" d="M1 8a7 7 0 1 0 14 0A7 7 0 0 0 1 8m15 0A8 8 0 1 1 0 8a8 8 0 0 1 16 0m-4.5-.5a.5.5 0 0 1 0 1H5.707l2.147 2.146a.5.5 0 0 1-.708.708l-3-3a.5.5 0 0 1 0-.708l3-3a.5.5 0 1 1 .708.708L5.707 7.5z" />
          </svg><p className="h6 mb-0 ms-2" onClick={goBackClick} style={{ cursor: 'pointer' }}>Go Back</p></div>
        ...
        <form action="" id="frmregisterprofile" onSubmit={onSubmit}>
            ...
            <div className="col-md-12">
              ...
              <div className="row">
                ...
                <div className="form-group col-md-4">
                  ...
                  <input type="text" className="form-control border border-secondary" name="sex" id="sex" value={Sex} onChange={onchange}></input>
                </div>
              ...
              <div className="row">
                ...
                <div className="form-group col-md-4">
                  ...
                  <DatePicker type="text" className="form-control border border-secondary" name='DOB' id="DOB" value={formatDate(DOB1)}
                    selected={DOBBirth}
            ...
            <input type="text" className= "form-control border border-secondary" name="additionalQualification" id="additionalQualification" placeholder="" onChange={onchange} aria-label=".form-select-sm example"></input>
              ...
              <div className="row">
                ...
                <div className="form-group col-md-4">
                  <label htmlFor="fatherOccupation">Father Occupation </label>
                   <input type="text" className="form-control border border-secondary" name="fatherOccupation" id="fatherOccupation" onChange={onchange}></input>
                  {/* <select className="form-select form-control-sm custom-select border border-secondary" name="fatherOccupation" id="fatherOccupation" onChange={onchange} aria-label=".form-select-sm example">
                    <option value="">Select</option>
                    {((jobs != null && jobs.length > 0) &&
                      jobs.map((job) => (
                        <option key={job._id} value={job.job}>{job.job}</option>
              ...
              <div className="row">
                ...
                <div className="form-group col-md-4">
                  ...
                  {/* <select className="form-select form-control-sm custom-select border border-secondary" name="motherOccupation" id="motherOccupation" onChange={onchange} aria-label=".form-select-sm example">
                    <option value="">Select</option>
                    {((jobs != null && jobs.length > 0) &&
                      jobs.map((job) => (
                        <option key={job._id} value={job.job}>{job.job}</option>
              ...
              <div className="row">
                ...
                <div className="form-group col-md-4">
                  ...
                  {/* <input type="text"  className= "form-control border border-secondary" name="district" id="district" onChange={onchange}></input> */}
                  <input type="text" className="form-control border border-secondary" name="district" id="district" value={District} onChange={onchange}></input>
                  {/* <select className="form-select form-control-sm custom-select border border-secondary" name="district" id="district" value={District} onChange={onchange} aria-label=".form-select-sm example">
                    ...
                    {((districts != null && districts.length > 0) &&
                      ...
                      districts.map((district) => (value={District} 
          ...
          <div>
            <div className="button-container">
              ...
              <button onClick={() => goBackClick()} className="secondarybutton">Cancel</button>
            </div>
          </div>
        </form>
      </div>
